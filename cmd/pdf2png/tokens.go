...
// extractFavorTokensFromPDF extracts the images for favor tokens from a PDF file and saves them as individual PNG files
func extractFavorTokensFromPDF(doc *fitz.Document, outputDir string) error {
	const cardWidth, leftMargin, rightMargin = 1048, 149, 149
	const cardHeight, topMargin, bottomMargin = 750, 149, 450
	const rows, cols = 8, 5
	...
	for pageNum := 0; pageNum < doc.NumPage(); pageNum++ {
		...
		for row := 0; row < rows; row++ {
			for col := 0; col < cols; col++ {
				tile := tiles[row][col]
				tileImagePath := filepath.Join(outputDir, fmt.Sprintf("token_%02d_%02d_%02d.png", pageNum+1, row+1, col+1))
	...
	log.Printf("pdf: tokens have been successfully extracted and saved.\n")
