...
// extractActorCardsFromPDF extracts the images for play cards from a PDF file and saves them as individual PNG files
func extractActorCardsFromPDF(doc *fitz.Document, outputDir string) error {
	...
	for pageNum := 0; pageNum < doc.NumPage(); pageNum++ {
		...
		for row := 0; row < rows; row++ {
			for col := 0; col < cols; col++ {
				tile := tiles[row][col]
				tileImagePath := filepath.Join(outputDir, fmt.Sprintf("actor_%02d_%02d_%02d.png", pageNum+1, row+1, col+1))
	...
	log.Printf("pdf: actor cards have been successfully extracted and saved.\n")
